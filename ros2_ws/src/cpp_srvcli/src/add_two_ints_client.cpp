...
int main(int argc, char **argv)
{
  ...
  request->a = atoll(argv[1]);      // Estructura definida por el archivo .srv
  request->b = atoll(argv[2]);

  while (!client->wait_for_service(1s)) {   // Gives the client 1s to search for the service nodes in the network
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),      // When Ctrl+C is pressed for cancelling the client
